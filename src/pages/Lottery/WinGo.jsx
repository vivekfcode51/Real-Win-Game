...
const notes = [
  "Notice:To visit our official website, be sure to use the link below,https://Real_win.com / Please re",
  "Notice:To visit our official website, be sure to use the link below,https://Real_win.com / Please re",
  "Notice:To visit our official website, be sure to use the link below,https://Real_win.com / Please re",
];
const WinGo = () => {
  ...
  const gameHistory = async () => {
    ...
    try {
      ...
      console.log("result ahi res",res)
      if (res?.data?.status === 200) {
          ...
          <div className='bg-gradient-to-l from-[#ff9a8e] to-[#f95959] h-[19rem] rounded-b-[55px] px-4 pt-2'>
            ...
            <div className='flex  justify-between p-3 mt-5 rounded-2xl' style={{
              ...
              <div className='w-[50%]'>
                ...
                <p className='flex justify-end text-sm 3xl:text-[1.2rem] font-extrabold mt-3'>{Number(gameHistoryData[0]?.games_no) + 1}</p>
              </div>
            </div>
          </div>
