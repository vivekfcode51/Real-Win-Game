...
const notes = [
  // "Notice:To visit our official website, be sure to use the link below,https://upcolor.live/ / Please re",
  // "Notice:To visit our official website, be sure to use the link below,https://upcolor.live/ / Please re",
  // "Notice:To visit our official website, be sure to use the link below,https://upcolor.live/ / Please re",
  "Please be sure to always use our official website for playing the games with the fol",
  "If your deposit is not received, Please send it directly to Real_Win Games Self-service Ce"
];
const TrxWinGo = () => {
  ...
  const winAmountAnnouncement1 = async (i) => {
    console.log("1111111111111111111111111111111111111111111111111111111",i)
    try {
      const offset = (currentPage - 1) * limit;
      const res = await axios.get(
        `https://trx.apponrent.com/api/trx/result?gameid=${i}&limit=${limit}&offset=${offset}`
      );
  ...
  const winAmountAnnouncement2 = async (i) => {
    console.log("22223333333222222222222222222")
    try {
      const offset = (currentPage - 1) * limit;
      const res = await axios.get(
        `https://trx.apponrent.com/api/trx/result?gameid=${i}&limit=${limit}&offset=${offset}`
  ...
  const myHistory = async () => {
    ...
    // console.log("my history wingo_my_history",wingo_my_history)
    try {
  ...
  // https://upcolor.live/api/trx/result?gameid=1&limit=10&offset=0
  const gameHistory = async () => {
    ...
    try {
      const offset = (currentPage - 1) * limit;
      const res = await axios.get(
        `https://trx.apponrent.com/api/trx/result?gameid=${gameDetails?.gameId}&limit=${limit}&offset=${offset}`
      );
